...
cover: //misc.aotu.io/JChehe/2016-11-14-xcel/poster.jpg  
categories: 项目总结
tags:
  - excel
  - data-clening
...
wechat:  
    share_cover: //misc.aotu.io/JChehe/2016-11-14-xcel/wx_share.jpg  
    share_title: XCel 项目总结 - Electron 与 Vue 的性能优化  
    share_desc: 本文将分享 Electron、Vue    等相关涉及技术的基本知识和性能优化经验。这也许是Jc的2016年最后一篇文章，且看且不珍惜。  
date: 2016-11-14 23:23:23
---

XCel 是一个对 Excel 数据进行可视化筛选的工具。它基于 Electron 和 Vue 实现，拥有跨平台（windows 7+、Mac 和 Linux）的特性。

落地页：http://jdc.jd.com/lab/xcel/   ✨✨✨  
项目地址：https://github.com/JChehe/XCel   ✨✨✨  
...
## 特性一览
 ...
 - 拥有 Excel 文件导入、数据清洗、导出的一站式体验
 - 拥有单列运算、多列运算和双列运算的逻辑，每个逻辑可任意组合
 - 拥有比 Web 更加原生的体验
...
至此，Electron 的基础部分介绍完毕。该部分是基于我之前翻译的一篇文章《Electron 的本质》，想更多地了解 Electron，可查看点击查阅。
...
Vue 1.x -> Vue 2.0 的版本迁移用 [vue-migration-helper][9] 即可分析出大部分需要更改的地方。
...
### js-xlsx
该库支持各种电子表格格式的解析和生成。它由纯 JavaScript 实现，适用于前端和 Node。[详情>>][10]
...
更多关于 Excel 在 JavaScript 中处理的知识可关注：凹凸实验室的[《Node读写Excel文件探究实践》][11]。但该文章存在两处问题（均在 js-xlsx 实战的导出表格部分）：
...
2. 转换成 worksheet 需要的结构处，出现逻辑性错误，并且会导致严重的性能问题。逻辑问题在此不讲述，我们讲下性能问题：
 ECMAScript 的不断更新，让 JavaScript 更加强大和易用。尽管如此，我们还是要做到『物尽所用』，而不要『大材小用』，否则会得到反效果。这里导致性能问题的正是 [Object.assign()][12] 方法，该方法可以把任意多个的源对象自身的可枚举属性拷贝给目标对象，然后返回目标对象。由于该方法自身的实现机制，会在这里产生大量的冗余操作。而这里的单元格信息是唯一的，所以直接通过 forEach 为一个空对象赋值即可。提升 N 倍性能的同时，也把逻辑性错误解决了。
...
### Excel 的列转换
 ...
 - Excel 的列需要用『字母』表示，但不能简单地通过 [String.fromCharCode()][14] 实现，因为当超出 `26列` 时会产生问题（如：第 `27` 列，`String.fromCharCode(65+26)` 得到的是 `[`，而不是 `AA`）。因此，这需要通过『十进制和26进制转换』算法来实现。
...
### 支持常用的编辑功能，如粘贴和复制
Electron 应用在 MacOS 中默认不支持『复制』『粘贴』，因此需要为 MacOS 显式地设置复制粘贴等编辑功能的菜单栏，并为此设置相应的快捷键。
...
其实现方式，可参考[《4 must-know tips for building cross platform Electron apps》][15]。
...
> 在浏览器当中，JavaScript 的运算在现代的引擎中非常快，但 DOM 本身是非常缓慢的东西。当你调用原生 DOM API 的时候，浏览器需要在 JavaScript 引擎的语境下去接触原生的 DOM 的实现，这个过程有相当的性能损耗。所以，本质的考量是，要把耗费时间的操作尽量放在纯粹的计算中去做，保证最后计算出来的需要实际接触真实 DOM 的操作是最少的。 —— [《Vue 2.0——渐进式前端解决方案》][16]
...
1. storage 事件：对某个标签页的 localStorage 对象进行增删改时，其他标签页能通过 window.storage 事件监听到。
2. 通过主进程作为中转站：设主界面的渲染进程是 A，`background process` 是 B，那么 A 先将 Excel 数据传递到主进程，然后主进程再转发到 B。B 处理完后再原路返回。
...
a、首次打开页面（第 4 行是主进程；第 1 行是页面渲染进程；第 3 行是 background 渲染进程 ）    
![image](http://7xq7nb.com1.z0.glb.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202016-10-22%20%E4%B8%8B%E5%8D%886.12.03.png)  

b、导入文件（第 5 行是主进程；第 2 行是页面渲染进程；第 4 行是 background 渲染进程 ）   
![image](http://7xq7nb.com1.z0.glb.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202016-10-22%20%E4%B8%8B%E5%8D%886.12.33.png)  

c、筛选数据（第 4 行是主进程；第 1 行是页面渲染进程；第 3 行是 background 渲染进程 ）   
![image](http://7xq7nb.com1.z0.glb.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202016-10-22%20%E4%B8%8B%E5%8D%886.13.20.png)   

由于 JS 目前不具有主动回收资源的功能，所以只能主动将对象设置为 `null`，然后等待 GC 回收。

因此，经过一段时间等待后，内存占用如下：   
d、一段时间后（第 4 行是主进程；第 1 行是页面渲染进程；第 3 行是 background 渲染进程 ）
![image](http://7xq7nb.com1.z0.glb.clouddn.com/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202016-10-22%20%E4%B8%8B%E5%8D%886.24.43.png)
...
最后，感谢 [LV][19] 的产品规划、界面设计和优化上的强力支持。全文完！


  [1]: http://electron.atom.io/
  [2]: http://vuejs.org/
  [3]: https://github.com/SheetJS/js-xlsx
  [4]: //misc.aotu.io/JChehe/2016-11-14-xcel/main.jpg
  [5]: //misc.aotu.io/JChehe/2016-11-14-xcel/renderer.jpg
  [6]: //misc.aotu.io/JChehe/2016-11-14-xcel/like-this.jpg
  [7]: //misc.aotu.io/JChehe/2016-11-14-xcel/ipc.jpg
  [8]: //misc.aotu.io/JChehe/2016-11-14-xcel/app-files.jpg
  [9]: https://github.com/vuejs/vue-migration-helper
  [10]: https://github.com/SheetJS/js-xlsx
  [11]: https://aotu.io/notes/2016/04/07/node-excel/
  [12]: https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Object/assign
  [13]: //misc.aotu.io/JChehe/2016-11-14-xcel/fenge.jpg
  [14]: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/fromCharCode
  [15]: https://blog.avocode.com/blog/4-must-know-tips-for-building-cross-platform-electron-apps
  [16]: http://www.infoq.com/cn/articles/vue-2-progressive-front-end-solution
  [17]: http://blog.csdn.net/luoshengyang/article/details/47822689
  [18]: //misc.aotu.io/JChehe/2016-11-14-xcel/neicun5.png
  [19]: https://github.com/mamboer
