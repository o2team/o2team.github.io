...
基于以上痛点，羚珑平台需要业务网关与网关控制台来管理、限流、监控、权限校验与登录认证等，并且我们为该服务起了个代号：**Monet**，Monet 取自于
...
所以我们最终以 Spring Cloud Gateway 基础框架
...
在此处主要扩展五种 Filter 分别是：AccessLog、API 监控、API 解析、限流熔断、登录认证、权限校验；
...
Spring Cloud Gateway 可以根据流行的几种算法实现限流效果，Monet 是新项目并且开发周期时间不多，所以直接找开源的限流熔断组件，流行的组件有：Hystrix 与 Sentinel[6]，两个组件区别：
