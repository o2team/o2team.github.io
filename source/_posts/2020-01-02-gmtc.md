title: GMTC 2019 深圳《小程序跨框架开发的探索与实践》演讲稿 
...
* **Learn Once Write AnyWhere & Write Once Run AnyWhere**：很多开发者更喜欢 React 提出的`Learn Once Write AnyWhere`，而我们 Taro 的口号是 `Write Once Run AnyWhere`，这一点也经常导致我们经常被人喷，这里说一点我自己的想法：`Learn Once Write AnyWhere`其实本质上对开发者更友好，比如开发者只需要学习 React 技术栈，就可以开发 Web/移动端 应用，但是对项目就没那么友好了，每个项目都得维护一份代码；而 `Write Once Run AnyWhere` 是对开发者没那么友好（适配的端越多，适配的成本必然也会水涨船高，对开发者要求也很变高），但是根据我们的实践，对项目会更友好，「一套代码，多端适配」。当然，这里适配的粒度，并不一定是项目级别的，其实在我们的具体实践中，有相当一部分是：**业务级甚至是页面级的**。
