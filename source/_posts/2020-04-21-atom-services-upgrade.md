...
这次升级按照功能和通用性把项目划分为 17 个模块：

| 模块名 | 描述 |
| --- | --- |
| 邮件（通用） | 提供邮件发送功能 |
| 编译（通用） | 提负责对页面、小程等进行编译 |
| 数据池 | 对数据池进行管理 |
| 发布（通用） | 提供发布到通天塔、CMSX 和 ATP 的服务 |
| Atom 发布上线 | 为 Atom 提供发布上线功能，连接 Atom 和通用发布服务 |
| 编辑器 | 提供编辑器相关的管理 |
| 项目管理 | 提供项目和页面以及小程序等的管理服务 |
| 代理 | 提供请求转发功能，用于羚珑接口以及小程序二维码请求的转发 |
| 站点管理 | 对站点进行管理 |
| 标签管理 | 对标签进行管理 |
| 模板管理 | 对模板进行管理 |
| 文件上传 | 提供图片文件校验上传功能 |
| 版本管理 | 管理页面和模板的版本
| 审核（通用） | 提供通用的审核服务 |
| Atom 审核 | 为 Atom 提供审核功能，主要有页面发布上线和模板的审核等，连接 Atom 和通用审核服务 |
| 定时任务 | 系统定时任务，包括页面生效检测和 PV/UV 定时获取等任务 |
| 网关 | 系统接口入口，统一校验登录态，初步校验接口权限 |

其中拆分出 4 个通用服务，通用服务独立于 Atom 系统之外，可以为 Atom 以及其他系统提供服务。
